...
std::optional<bool> process_panda_state(std::vector<Panda *> &pandas, PubMaster *pm, bool engaged, bool is_onroad,
                         bool spoofing_started, bool ignore_ignition_line) {
...
void pandad_run(std::vector<Panda *> &pandas) {
  ...
  while (!do_exit && check_all_connected(pandas)) {
    ...
    if (rk.frame() % 10 == 0) {
      ...
      const auto ignition = process_panda_state(pandas, &pm, engaged, is_onroad, spoofing_started, ignore_ignition_line);
      // Do not latch the configured safety state while ignition is absent: send_panda_states sets
      // NO_OUTPUT in that state, and configureSafetyMode latches once -> stuck noOutput after ignition
      // returns. Only configure once we're onroad AND ignition is present.
      panda_safety.configureSafetyMode(is_onroad && ignition.value_or(false));
    }
