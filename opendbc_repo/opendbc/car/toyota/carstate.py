...
class CarState(CarStateBase):
  ...
  def update_tss3(self, can_parsers, starpilot_toggles):
    ...
    # 0x1A0 ADAS_STEER_COMMAND: gateway-sent, relayed onto the bus-2 ADAS parser (DBC: "relayed,
    # panda bus 0/2 ... EPS executes it"). Captured with float('nan') frequency in the cam parser so
    # an intermittent receive rate never fails cp_cam.can_valid (that was the earlier canValid
    # regression that blocked engagement). Fail-closed: if the message is absent the DBC bytes read
    # back all-zero, so we leave the template None and no steer frame is injected.
    if cp_cam.can_valid:
      steer = cp_cam.vl["ADAS_STEER_COMMAND"]
      steer_bytes = bytes(int(steer[f"BYTE{k:02d}"]) & 0xFF for k in range(48))
      self.tss3_steer_template = steer_bytes if any(steer_bytes) else None
    else:
