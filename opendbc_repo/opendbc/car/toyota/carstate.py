...
class CarState(CarStateBase):
  ...
  def update_tss3(self, can_parsers, starpilot_toggles):
    ...
    # 0x1A0 ADAS_STEER_COMMAND: gateway-sent, "relayed, panda bus 0/2 ... EPS executes it" (DBC).
    # Its ordinary-receive bus is INCONSISTENT between drives (seen ordinary-rx on bus 2 in one log,
    # only on bus 0 in another; the forwarded copy shows as a tx-return the CANParser does not read).
    # So capture from BOTH the bus-2 cam parser and a dedicated bus-0 adas parser, taking whichever
    # actually carries a non-zero frame. All reads use float('nan') frequency so a missing rate never
    # fails can_valid (the earlier canValid regression). Fail-closed: if neither bus has it the bytes
    # read back all-zero -> template stays None -> no steer frame is injected.
    steer_template = None
    for cp_s in (cp_cam, cp_adas):
      if cp_s is None or not cp_s.can_valid:
        continue
      steer = cp_s.vl["ADAS_STEER_COMMAND"]
      steer_bytes = bytes(int(steer[f"BYTE{k:02d}"]) & 0xFF for k in range(48))
      if any(steer_bytes):
        steer_template = steer_bytes
        break
    self.tss3_steer_template = steer_template
  ...
  def get_can_parsers(CP):
    if CP.flags & ToyotaFlags.CAN_FD.value:
      ...
      return {
        ...
        Bus.cam: CANParser(DBC[CP.carFingerprint][Bus.pt], [("ADAS_ACC_REQUEST", 40), ("ADAS_STEER_COMMAND", float('nan'))], 2),
        # Dedicated bus-0 reader for the gateway-native 0x1A0 (ADAS_STEER_COMMAND), which is on bus 0
        # in some drives and bus 2 in others. float('nan') only -> never affects canValid/timeout.
        Bus.adas: CANParser(DBC[CP.carFingerprint][Bus.pt], [("ADAS_STEER_COMMAND", float('nan'))], 0),
      }
