...
class CarState(CarStateBase):
  ...
  def update_tss3(self, can_parsers, starpilot_toggles):
    ...
    # 0x1A0 ADAS_STEER_COMMAND is NOT read here: it arrives on bus 0 (not the bus-2 camera
    # parser) and its bus is inconsistent, so requiring it broke cp_cam.can_valid -> carState
    # canValid=false -> openpilot could not engage. Steering (0x1A0) is deferred until its
    # receive bus/topology is proven; leave the template unset so no steer is injected.
