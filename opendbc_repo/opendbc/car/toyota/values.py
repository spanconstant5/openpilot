...
FW_QUERY_CONFIG = FwQueryConfig(
  ...
  requests=[
    # TSS3 Corolla firmware responds to UDS 22 F1 00 on the OBD bus (bus 1), response prefix 0x62
    # (keeps F1 00 in the version bytes). Without a Toyota request here the ECUs only answer the
    # Hyundai 22 f1 00 query, so the FW is filed under brand 'hyundai' and the Toyota matcher
    # discards it before comparing -> AUTO fingerprint fails. This makes the same query under Toyota.
    Request(
      [b'\x22\xf1\x00'],
      [b'\x62'],
      whitelist_ecus=[Ecu.hvac, Ecu.fwdRadar, Ecu.fwdCamera],
      bus=1,
    ),
    Request(
...
# Lateral goes out on 0x1A0 (ADAS_STEER_COMMAND). LIVE asserts STEER_REQUEST (byte6 0x40) so
...
# Steering re-enabled 2026-09-16 (carstate captures the 0x1A0 template again). START IN SHADOW:
# forged STEER_REQUEST actuation, angle sign, and the ~-25 deg angle offset are UNCONFIRMED on-car.
# SHADOW emits valid 0x1A0 frames with STEER_REQUEST off (NO wheel movement) so we can verify from
# logs that the commanded angle tracks the desired path and direction. Once confirmed, flip this one
# line to TSS3LatMode.LIVE and ramp TSS3_MAX_STEER_ANGLE up from 15 deg. Do NOT go LIVE at 50 deg first.
TSS3_LAT_MODE = TSS3LatMode.SHADOW  # set back to SHADOW: this is the FIRST drive 0x1A0 actually emits
TSS3_LAT_RELAY_ONLY = False
TSS3_MAX_STEER_ANGLE = 15.0       # deg steering-wheel authority cap; raise gradually AFTER a clean LIVE low-angle test
