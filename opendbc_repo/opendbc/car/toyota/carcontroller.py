...
class CarController(CarControllerBase):
  ...
  def update(self, CC, CS, now_nanos, starpilot_toggles):
    ...
    if self.CP.flags & ToyotaFlags.CAN_FD.value:
      # openpilot stays PASSIVE unless actively controlling: the camera's 0x160/0x1A0 flow to the
      # car unblocked (so DRCC/AEB and stock functions keep working), and openpilot injects its own
      # 0x160 only while long-controlling and 0x1A0 only while lat-commanding. Nothing is asserted
      # while disengaged / on normal cruise.
      # Kev-ORG PR #3791 scheme: BOTH accel and steer ride in the camera's 0x160 frame. openpilot is
      # the SOLE writer of 0x160 whenever ACC is engaged (no CAN-level handoff -- handoffs at stops
      # tripped the gateway). We emit exactly one frame per NEW camera counter (~40 Hz), stamped with
      # the camera's own counter for a seamless E2E sequence. Steer lives in bytes 22-23 (537.7
      # counts/deg); the abandoned 0x1A0 path is gateway-native and trips relayMalfunction -- not used.
      template = CS.tss3_accel_template
      cam_counter = template[2] if template is not None else None

      # engaged_160: ACC on + OP long enabled + template present -> OP owns the 0x160 stream.
      engaged_160 = (TSS3_LONG_MODE == TSS3LongMode.LIVE and self.CP.openpilotLongitudinalControl and
                     CC.enabled and CS.out.cruiseState.enabled and not CS.out.gasPressed and
                     template is not None)
      long_controlling = engaged_160 and CC.longActive and CS.out.vEgo > TSS3_MIN_OVERRIDE_SPEED
      # LIVE only actually steers; SHADOW relays the camera's steer field (validate angle from logs first).
      lat_commanding = (TSS3_LAT_MODE == TSS3LatMode.LIVE and CC.enabled and CC.latActive and
                        not TSS3_LAT_RELAY_ONLY)

      applied_accel = 0.0
      applied_angle = 0.0

      if engaged_160 and cam_counter != self.tss3_last_cam_counter:
        self.tss3_last_cam_counter = cam_counter
        # accel: substitute OP's when controlling, else relay the camera's (None).
        accel_cmd = float(np.clip(actuators.accel, -1.5, 1.5)) if long_controlling else None
        applied_accel = accel_cmd if accel_cmd is not None else 0.0
        # steer: substitute OP's clamped+slewed angle when LIVE-commanding, else relay the camera's (None).
        angle_cmd = None
        if lat_commanding:
          ...
          self.tss3_applied_angle = float(np.clip(target,
                                                  ...
                                                  self.tss3_applied_angle + TSS3_MAX_STEER_ANGLE_RATE))
          angle_cmd = self.tss3_applied_angle
          applied_angle = self.tss3_applied_angle
        can_sends.append(toyotacan.modify_tss3_160(template, accel_cmd, angle_cmd, cam_counter))
      elif not engaged_160:
        self.tss3_last_cam_counter = cam_counter
