...
static bool toyota_fwd_hook(int bus_num, int addr) {
  ...
  // false-trip stock_ecu_check -> relayMalfunction). The camera's 0x160 is NEVER blocked so the
  // car's DRCC/AEB keep working at full rate. The camera's 0x1A0 is blocked ONLY while openpilot
  // is actively steering (tss3_1a0_relay_block armed in the tx hook), so the EPS follows a single
  // steer source (fixes controlsMismatch); it self-expires when openpilot stops steering.
